...
describe('widget-bbm-initiate-payment-ng::FormController', () => {
  const widget = {
    getId: () => '123',
  };
  ...
  const createController = (preferences = sharedApi.preferences) => new FormController(
    widget,
    model,
    viewModel,
    Object.assign({}, sharedApi, { preferences }),
    bbIntent,
    bus,
    hooks,
    Promise
    ...
    describe('#submitPayment', () => {
      describe('The widget has the review step', () => {
        it('should show the payment review', done => {
          ...
          const ctrl = createController({ reviewStep: true });
  ...
  describe('Intents', () => {
    ...
    describe('view.payment.form.show', () => {
      it('should reset the form', done => {
        const intentHandlers = mockIntentHandlers();
        const ctrl = createController();
        ...
        ctrl.$onInit()
          ...
          .then(() => {
            ...
            intentHandlers['view.payment.form.show']({ reset: true });
          })
          .then(() => {
            expect(viewModel.setPayment).toHaveBeenCalledWith({
              amount: 100,
    ...
    describe('intent.bb.paymentOrder.payment.initiate', () => {
      it('should initiate a new payment', done => {
        const intentHandlers = mockIntentHandlers();
        const ctrl = createController();
        ...
        ctrl.$onInit()
          ...
          .then(() => {
            ...
            intentHandlers['intent.bb.paymentOrder.payment.initiate']();
          })
          .then(() => {
            expect(viewModel.setPayment).toHaveBeenCalledWith({
              amount: 100,
      ...
      it('should prepopulate the payment form with a debit account that was sent with the intent', done => {
        const intentHandlers = mockIntentHandlers();
        const ctrl = createController();
        ...
        ctrl.$onInit()
          ...
          .then(() => {
            ...
            expect(viewModel.setSelectedDebitAccount).toHaveBeenCalledWith(debitAccount);
          })
          .then(done, done.fail);
      });
    });
