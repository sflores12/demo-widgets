...
export default (widget, model, viewModel, bus, hooks, Promise) => {
  ...
  const getContactIdentification = beneficiary => ({
    counterpartyName: beneficiary ? beneficiary.name : null,
  ...
  /**
   * @description
   * Returns the identification of the given beneficiary.
   *
   ...
   * @inner
   */
  const getBeneficiaryIdentification = creditAccount => (
    isExternalBeneficiary(creditAccount)
      ? getContactIdentification(creditAccount)
      : getCreditAccountIdentification(creditAccount)
  );

  /**
   * @description
   * Returns the identification of the given debit account.
   *
   * @name getDebitAccountIdentification
   * @type {function}
   *
   * @param {AccountView} debitAccount
   * @returns {DebitAccountIdentification}
   * @inner
   */
  const getDebitAccountIdentification = debitAccount => ({
    identification: debitAccount ? debitAccount.id : null,
    scheme: 'ID',
  });
  ...
  const getPaymentPayload = payment => {
    const { additions, beneficiary, debitAccount, description, schedule, urgent } = payment;
    const { currency, value: amount } = payment.amount;

    const debitAccountIdentification = getDebitAccountIdentification(debitAccount);
    const creditAccountIdentification = getBeneficiaryIdentification(beneficiary);

    const paymentMode = isRecurring(schedule) ? PaymentMode.RECURRING : PaymentMode.SINGLE;
  ...
  const makePayment = () => {
    ...
    return model.makePayment(payload)
      .then(() => {
