...
describe('widget-bbm-payment-ng::shared-api', () => {
  const widget = {};
  ...
  describe('#makePayment', () => {
    it('should make a payment', done => {
      ...
      spyOn(model, 'makePayment').and.callThrough();

      api.makePayment()
        .then(() => {
          expect(model.makePayment).toHaveBeenCalledWith({
            debitAccountIdentification: { identification: 'account-b', scheme: 'ID' },
            creditAccountIdentification: { identification: 'account-a', scheme: 'ID' },
            amount: 100,
            currency: 'EUR',
            description: 'Test',
            date: '2015-05-15T14:00:00Z',
    ...
    it('should fail when payment is failed', done => {
      ...
      spyOn(model, 'makePayment').and.returnValue(Promise.reject('Error'));
      spyOn(bus, 'publish');
  ...
  describe('#makePaymentWithAuthorization', () => {
    ...
    it('should make a payment once the pin verification is successful', done => {
      ...
      spyOn(model, "makePayment").and.returnValue(Promise.resolve());
      ...
      api.makePaymentWithAuthorization()
        .then(() => {
          expect(model.makePayment).toHaveBeenCalled();
          done();
        })
        .catch(done.fail);
